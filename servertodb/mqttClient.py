...
class MqttClient(mqtt.Client):
    ...
    def on_message_methane(self, mqttc, obj, msg):
        print(msg.topic+" "+str(msg.qos)+" "+str(msg.payload))
        methane_data = Data(-1,-1,-1,datetime.now(),-1) # default object since we are parsing from JSON
        try:
    ...
    def on_message_amonia(self, mqttc, obj, msg):
        print(msg.topic+" "+str(msg.qos)+" "+str(msg.payload)) 
        amonia_data = Data(-1,-1,-1,datetime.now(),-1) # default object since we are parsing from JSON
        try:
            amonia_data.parseJSON(msg) 
    ...
    def on_message_hydrogen_sulfide(self, mqttc, obj, msg):
        print(msg.topic+" "+str(msg.qos)+" "+str(msg.payload)) 
        h2so4_data = Data(-1,-1,-1,datetime.now(),-1) # default object since we are parsing from JSON
        try:
            h2so4_data.parseJSON(msg) 
    ...
    def on_message_humidity(self, mqttc, obj, msg):
        print(msg.topic+" "+str(msg.qos)+" "+str(msg.payload))
        humidity_data = Data(-1,-1,-1,datetime.now(),-1) # default object since we are parsing from JSON
        try:
            humidity_data.parseJSON(msg) 
    ...
    def on_message_carbon_dioxide(self, mqttc, obj, msg):
        print(msg.topic+" "+str(msg.qos)+" "+str(msg.payload))
        co2_data = Data(-1,-1,-1,datetime.now(),-1) # default object since we are parsing from JSON
        try:
            co2_data.parseJSON(msg) 
    ...
    def on_message_temperature(self, mqttc, obj, msg):
        print(msg.topic+" "+str(msg.qos)+" "+str(msg.payload))
        temperature_data = Data(-1,-1,-1,datetime.now(),-1) # default object since we are parsing from JSON
        try:
            temperature_data.parseJSON(msg) 
