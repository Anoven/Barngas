				...
				<div className="card-header">
					<div className = 'row align-items-center'>
						<div className = 'col-sm-3'>
				  			<label className ="input-group-text bslabel-lg-2"> {'Basestation '+this.state.id + ": "} </label>
				  		</div>
...
export class CardContainer extends React.Component<{dict: {[id: number]: Basestation}}, {dict: any}> {
	...
	renderCards(): Array<JSX.Element> {
		...
		for(let id in this.state.dict) {
			cards.push(<Card id = {Number(id)} name = {this.state.dict[id].name} descr = {this.state.dict[id].description}/>);
		}
		return cards;
	}
	render() {
