        ...
        if(mqtt_client != None and data_client != None):
            if(not mqtt_client.base_cmds.empty()):
                ...
                if (parsedCmd.command == "update"):
                    ...
                    while (timeout>0):
                        if (data_client.data_points.empty()): # wait till all the data points are recorded and then we can update firmware
    ...
    while True:
        if (data_client!=None):
            if(not data_client.data_points.empty()):
                mqtt_client.publish_sensor_reading([data_client.data_points.get()])
